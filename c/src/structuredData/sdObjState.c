...
)
{
	...
	)
	{
		...
		t_atom outArray[5];
		SETSYMBOL( & outArray[0], gensym( "info" ) );
	...
	)
	{
		...
		t_atom outArray[5];
		SETSYMBOL( & outArray[0], gensym( "update" ) );
		SETFLOAT( & outArray[1], 3 );
		SETSYMBOL( & outArray[2], x->name );
		SETFLOAT( & outArray[3], 1 );
		SETFLOAT( & outArray[4], x->value );
